...
-- ORACLE TABLE SUBSCRIPTION_MEMBER
create table SUBSCRIPTION_MEMBER
(
    id              number generated by default as identity primary key,
    user_id         number    not null,
    subscription_id number    not null,
    role_status     varchar2(20) check ( role_status in ('OWNER', 'MEMBER')),
    accept          number(1) not null,
    invited_at      timestamp with time zone,

    constraint fk_sub_member_userid foreign key (user_id) references USERS,
    constraint fk_sub_member_subid foreign key (subscription_id) references SUBSCRIPTION
);
...
create sequence SEQ_SUBSCRIPTION_MEMBER;
